...
class PufferGPUDrive:
    def __init__(
        self, device='cuda', max_cont_agents=64, num_worlds=64, k_unique_scenes=1
    ):
        self.device = device
        ...
        self.actions = torch.zeros((num_worlds, max_cont_agents), dtype=torch.int64).to(
            self.device
        )
    ...
    def reset(self, seed=None, options=None):
        ...
        self.episode_returns = torch.zeros(self.num_agents, dtype=torch.float32).to(
            self.device
        )
        self.episode_lengths = torch.zeros(self.num_agents, dtype=torch.float32).to(
            self.device
        )
            ...
            info.append(
                {
                    ...
                    'mean_reward_per_episode': self.episode_returns[done_worlds]
                    .mean()
                    .item(),
