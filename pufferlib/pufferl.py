...
class PuffeRL:
    ...
    def train(self):
        ...
        var_y = y_true.var()
        explained_var = (
            torch.nan if var_y == 0 else (1 - (y_true - y_pred).var() / var_y).item()
        )
...
def train(
    env_name, args=None, vecenv=None, policy=None, logger=None, early_stop_fn=None
):
        ...
        if logs is not None:
            ...
            if early_stop_fn is not None:
                ...
                if 'early_stop_threshold' in logs:
                    pufferl.logger.log(
                        {
                            'environment/early_stop_threshold': logs[
...
def make_parser():
    ...
    parser.add_argument(
        '--no-model-upload',
        action='store_true',
        help='Do not upload models to wandb or neptune',
    )
    parser.add_argument(
...
def process_config(config, parser=None):
    ...
    parser.description = (
        f':blowfish: PufferLib [bright_cyan]{pufferlib.__version__}[/]'
