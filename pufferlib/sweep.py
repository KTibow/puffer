    ...
    def __init__(self, config, verbose=True):
        ...
        if verbose:
            ...
            for name, space in self.flat_spaces.items():
                print(
                    f'\t{name}: {space.unnormalize(max(space.norm_mean - space.scale, space.norm_min))}'
                )

            print('Max random sample:')
            for name, space in self.flat_spaces.items():
                print(
                    f'\t{name}: {space.unnormalize(min(space.norm_mean + space.scale, space.norm_max))}'
                )
...
def prune_pareto_front(
    pareto, efficiency_threshold=0.5, pruning_stop_score_fraction=0.98
):
...
class ParetoGenetic:
    ...
    def suggest(self, fill=None):
        ...
        if self.bias_cost:
            if self.log_bias:
                cost_dists = np.abs(
                    np.log(pareto_costs[:, None]) - np.log(pareto_costs[None, :])
                )
            else:
                cost_dists = np.abs(pareto_costs[:, None] - pareto_costs[None, :])

            cost_dists += (np.max(pareto_costs) + 1) * np.eye(
                len(pareto_costs)
            )  # mask self-distance
...
def train_gp_model(
    model, likelihood, mll, optimizer, train_x, train_y, training_iter=50
):
    model.train()
    likelihood.train()
...
class Protein:
    def __init__(
        ...
        _prune_pareto = (
            sweep_config['prune_pareto']
            if 'prune_pareto' in sweep_config
            else prune_pareto
        )
        _max_suggestion_cost = (
            sweep_config['max_suggestion_cost']
            if 'max_suggestion_cost' in sweep_config
            else max_suggestion_cost
        )
        ...
        if self.cost_param_idx is not None:
            self.cost_random_suggestion = (
                -0.8
            )  # In norm cost space. Make arg if necessary
        ...
        self.stop_threshold_model = RobustLogCostModel(
            quantile=sweep_config['early_stop_quantile']
        )
    ...
    def _sample_observations(self, max_size=None, recent_ratio=0.5):
        ...
        self.log_c_max = np.quantile(
            log_c, 0.97
        )  # Make it less sensitive to outlier points
    ...
    def _train_gp_models(self):
        ...
        y_norm = (y - self.min_score) / (
            np.abs(self.max_score - self.min_score) + EPSILON
        )
        ...
        log_c_norm = (log_c - self.log_c_min) / (
            self.log_c_max - self.log_c_min + EPSILON
        )
    ...
    def _sample_target_cost_ratio(
        ...
        target_ratio = np.clip(
            self.target_cost_ratio.pop() + 0.1 * np.random.randn(), 0, 1
        )
    ...
    def suggest(self, fill):
        ...
        if self.suggestion_idx <= self.num_random_samples:
            ...
            if self.cost_param_idx is not None:
                ...
                suggestion[self.cost_param_idx] = np.clip(
                    cost_suggestion, -1, 1
                )  # limit the cost
            return self.hyperparameters.to_dict(suggestion, fill), info
            ...
            and self.suggestion_idx % self.resample_frequency == 0
        ):
            candidates, _ = pareto_points(self.success_observations)
        ...
        self.stop_threshold_model.fit(
            self.success_observations, self.upper_cost_threshold
        )
        ...
        if len(suggestions) == 0:
            return self.suggest(
                fill
            )  # Fallback to random if all suggestions are filtered
        ...
        with (
            ...
            for i in range(0, len(suggestions), self.infer_batch_size):
                ...
                try:
                    ...
                    pred_y_mean = self.likelihood_score(
                        self.gp_score(batch_tensor)
                    ).mean.cpu()
                    pred_c_mean = self.likelihood_cost(
                        self.gp_cost(batch_tensor)
                    ).mean.cpu()
        ...
        if (
            ...
            and len(self.success_observations) > 9
            and len(self.failure_observations) > 9
        ):
