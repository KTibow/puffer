...
def emulate_observation_space(space):
    ...
    emulated_space = gymnasium.spaces.Box(
        low=mmin, high=mmax, shape=(numel,), dtype=dtype
    )
...
class GymnasiumPufferEnv(gymnasium.Env):
    def __init__(
        self, env=None, env_creator=None, env_args=[], env_kwargs={}, buf=None, seed=0
    ):
        ...
        self.is_obs_emulated = (
            self.single_observation_space is not self.env.observation_space
        )
...
class PettingZooPufferEnv:
    ...
    def step(self, actions):
        ...
        if isinstance(actions, np.ndarray):
            ...
            actions = {
                agent: actions[i] for i, agent in enumerate(self.possible_agents)
            }
        ...
        if not self.is_action_checked:
            ...
            self.is_action_checked = check_space(
                next(iter(actions.values())), self.single_action_space
            )
...
def make_object(
    object_instance=None, object_creator=None, creator_args=[], creator_kwargs={}
):
    if (object_instance is None) == (object_creator is None):
        raise ValueError(
            'Exactly one of object_instance or object_creator must be provided'
        )

    if object_instance is not None:
        if callable(object_instance) or inspect.isclass(object_instance):
            raise TypeError(
                'object_instance must be an instance, not a function or class'
            )
...
def _seed_and_reset(env, seed):
    ...
    try:
        ...
        try:
            ...
            obs, info = env.reset()
            warnings.warn(
                'WARNING: Environment does not support seeding.', DeprecationWarning
            )
...
class GymnaxPufferEnv(pufferlib.PufferEnv):
    ...
    def step(self, action):
        ...
        obs, self.state, reward, done, info = self.step_fn(
            self.rngs, self.state, action, self.env_params
        )
