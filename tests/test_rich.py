...
def print_dashboard(
    performance_data, loss_data, user_data, min_interval=0.25, last_print=[0]
):
    console = Console()
...
class Dashboard:
    ...
    def _update_utilization(self):
        ...
        self.utilization.update(
            self.dram_task, completed=psutil.virtual_memory().percent
        )
        gpus = GPUtil.getGPUs()
        if gpus:
    ...
    def update(
        ...
        topline = self.update_topline(
            total_uptime, estimated_time, total_steps, steps_per_second
        )
    ...
    def update_topline(
        self, total_uptime, estimated_time, total_steps, steps_per_second
    ):
    ...
    def update_user_stats(self, data):
        table = Table(box=None, expand=True)
        table.add_column(
            '[bold magenta]User Stats', justify='right', style='bold yellow', ratio=1.0
        )
