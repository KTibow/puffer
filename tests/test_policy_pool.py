...
class MockPolicy:
    def __call__(self, obs):
        ...
        values = (
            torch.arange(batch_size, dtype=torch.float32) + 10
        )  # add to make the values different
...
class TestPolicyPool(unittest.TestCase):
    ...
    def test_init_with_kernel(self):
        ...
        kernel = [0, 1, 0, 2]
        policy_ids, sample_idxs, kernel = test_policy_pool._init_sample_idx_from_kernel(
            kernel
        )

        self.assertTrue(np.array_equal(policy_ids, np.array([0, 1, 2])))
        self.assertEqual(
            sample_idxs, {0: [0, 2, 4, 6], 1: [1, 5], 2: [3, 7]}
        )  # map policy id to agent list
    ...
    def test_update_policies(self):
        ...
        policy_pool.update_policies(
            policy_ids=np.array([0, 1, 2, 3]), store=policy_store
        )
        self.assertEqual(
            len(set(p['name'] for p in policy_pool.current_policies.values())), 3
        )

        # Use all_selector
        policy_store = MockPolicyStore(5)
        policy_pool.update_policies(
            policy_ids=np.array([0, 1, 2, 3, 4, 5]),
            store=policy_store,
            policy_selector=pp.AllPolicySelector(seed=0),
        )
        self.assertEqual(
            len(set(p['name'] for p in policy_pool.current_policies.values())), 5
        )
    ...
    def test_update_scores(self):
        ...
        self.assertEqual(
            policy_infos['learner'], [{'return': 1}, {'return': 3}, {'return': 10}]
        )
