...
def profile_emulation(env_creator, timeout=DEFAULT_TIMEOUT, seed=42):
    ...
    env_overhead = (
        100
        * (puf_total_step - raw_total_step + puf_total_reset - raw_total_reset)
        / (puf_total_step + puf_total_reset)
    )
...
def profile_sb3_vec(env_creator, num_envs, timeout=DEFAULT_TIMEOUT):
    with pufferlib.utils.Suppress():
        ...
        actions = [
            [vecenv.action_space.sample() for _ in range(num_envs)] for _ in range(1000)
        ]
...
if __name__ == '__main__':
    ...
    # profile_emulation(env_creator)
    profile_puffer(
        env_creator, num_envs=24, batch_size=8, backend=Multiprocessing, zero_copy=False
    )
    ...
    profile_all(
        'Ocean Squared', ocean.env_creator('squared'), num_envs=1152, env_batch_size=48
    )
    ...
    profile_all(
        'Crafter', crafter.env_creator(), num_envs=24, env_batch_size=8, zero_copy=False
    )
