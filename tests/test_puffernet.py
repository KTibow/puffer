...
def test_puffernet_convolution_3d_layer(
    ...
    out_width = int((in_width - kernel_size) / stride + 1)
    output_puffer = np.zeros(
        (batch_size, out_channels, out_depth, out_height, out_width), dtype=np.float32
    )
...
def test_puffernet_lstm(batch_size=16, input_size=128, hidden_size=128):
    ...
    output_torch, (state_h_torch, state_c_torch) = torch_lstm(
        input_torch, (state_h_torch, state_c_torch)
    )
...
def test_puffernet_embedding(batch_size=16, num_embeddings=128, embedding_dim=32):
    ...
    puffernet.puf_embedding(
        input_np, weights_np, output_puffer, batch_size, num_embeddings, embedding_dim
    )
...
def test_puffernet_layernorm(batch_size=16, input_size=128):
    ...
    puffernet.puf_layernorm(
        input_np, weights_np, bias_np, output_puffer, batch_size, input_size
    )
...
def test_puffernet_argmax_multidiscrete(batch_size=16, logit_sizes=[5, 7, 2]):
    ...
    puffernet.puf_argmax_multidiscrete(
        input_np, output_puffer, batch_size, logit_sizes, num_actions
    )

    input_torch = torch.from_numpy(input_np)
    action_slices = torch.split(input_torch, logit_sizes.tolist(), dim=1)
    output_torch = torch.stack(
        [torch.argmax(s, dim=1) for s in action_slices], dim=1
    ).detach()
