...
def test_sweep(args):
    ...
    if method == 'Random':
        ...
        raise ValueError(
            f'Invalid sweep method {method} (random/pareto_genetic/protein)'
        )
...
def visualize(args):
    ...
    # Set up the figure
    p = figure(
        title='Synthetic Hyperparam Test', x_axis_label='Cost', y_axis_label='Score'
    )
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--vis-path', type=str, default='', help='Set to visualize a saved sweep'
    )
    parser.add_argument(
        '--data-path',
        type=str,
