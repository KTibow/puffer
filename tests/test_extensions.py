...
def test_pack_unpack():
    for space in nested_spaces:
        ...
        assert pufferlib.utils.compare_space_samples(sample, unflat_sample), (
            'Unflatten failed.'
        )
...
def test_flatten_unflatten(iterations=10_000):
    ...
    for space in test_cases:
        ...
        concatenate_times.append(
            timeit.timeit(lambda: concatenate(flat), number=iterations)
        )
        split_times.append(
            timeit.timeit(
        ...
        unflatten_times.append(
            timeit.timeit(lambda: unflatten(unmerged, structure), number=iterations)
        )
